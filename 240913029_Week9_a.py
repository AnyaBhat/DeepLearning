...
# Function to visualize VGG kernels/filters
def visualize_kernels(model):
    # Get the weights from the first conv layer
    kernels = model.features[0].weight.detach().cpu().numpy()
    ...
    for i, ax in enumerate(axes.flat):
        if i < kernels.shape[0]:  # We have 64 kernels in the first layer
...
def visualize_feature_maps(model, device, testloader):
    ...
    class FeatureExtractor(nn.Module):
        def __init__(self, model):
            ...
            self.features = nn.Sequential(
                model.features[0],  # First conv layer
                model.features[1],  # First batch norm
                model.features[2]  # First ReLU
            )
    ...
    for i, ax in enumerate(axes.flat):
        if i < feature_maps.shape[0]:  # We have 64 feature maps
